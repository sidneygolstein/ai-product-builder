...
Check `slice_type` in `ai/feature_list.json`:

- **`trivial` slices**: review inline in the main session (no spec-reviewer subagent). Verify:
  (1) the change is genuinely small and contained — no new files, no new logic, ~50 lines max;
  (2) at least one testable AC exists. On confirmation, update status to TO DO — Notion first
  (notion-board skill), then ai/feature_list.json.

- **`ui` and `backend` slices**: use the spec-reviewer subagent (you did NOT write these).
